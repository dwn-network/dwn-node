...
const { slots } = require('@dwn/extend')
const { Block } = require('@dwn/extend').models
...
module.exports = class Blockchain {
  ...
  constructor(networkStart) {
    ...
    if (this.state.networkStart) {
      logger.warn(
        'Dwn Core is launched in Genesis Start mode. This is usually for starting the first node on the blockchain. Unless you know what you are doing, this is likely wrong. :warning:',
      )
      logger.info('Starting Dwn Core for a new world, welcome aboard :rocket:')
    }
  ...
  async start(skipStartedCheck = false) {
    ...
    if (skipStartedCheck || process.env.DWN_SKIP_BLOCKCHAIN_STARTED_CHECK) {
      return true
    }

    // TODO: this state needs to be set after state.getLastBlock() is available if DWN_ENV=test
