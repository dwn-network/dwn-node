...
const { configManager } = require('@dwn/extend')
...
class ConfigLoader {
  ...
  async setUp(options) {
    ...
    this.network = JSON.parse(process.env.DWN_NETWORK)
  ...
  async copyFiles(dest) {
    if (!dest) {
      dest = `${process.env.DWN_PATH_DATA}/config`
    }

    await fs.ensureDir(dest)

    return fs.copy(process.env.DWN_PATH_CONFIG, dest)
  }
   ...
   */
  __getFiles() {
    const basePath = path.resolve(process.env.DWN_PATH_CONFIG)
