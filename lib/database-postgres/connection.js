...
const { ConnectionInterface } = require('@dwn/database')
...
const { roundCalculator } = require('@dwn/utils')
...
module.exports = class PostgresConnection extends ConnectionInterface {
  ...
  async buildWallets(height) {
    ...
    const spvPath = `${process.env.DWN_PATH_DATA}/spv.json`

    if (fs.existsSync(spvPath)) {
      fs.removeSync(spvPath)

      logger.info(
        'Dwn Core ended unexpectedly - resuming from where we left off :runner:',
      )
