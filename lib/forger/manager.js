...
const { Delegate, Transaction } = require('@dwn/extend').models
...
module.exports = class ForgerManager {
  ...
  // TODO 执行pm2 reload dwn-node命令
  __reload() {
    this.quorum = this.quorum + 1
    if (this.quorum > 3) {
      //TODO 连续出块失败3次后 执行pm2 reload dwn-node命令
      logger.info('exec:pm2 reload dwn-node')
      const cmd = 'pm2 reload dwn-node';
