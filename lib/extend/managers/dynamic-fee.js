...
class DynamicFeeManager {
   ...
   * @param {Number} Minimum fee DWNTOSHI/byte
   * @param {Transaction} Transaction for which we calculate the fee
   * @returns {Number} Calculated minimum acceptable fee in DWNTOSHI
   */
  calculateFee(dwntoshiPerByte, transaction) {
    if (dwntoshiPerByte <= 0) {
      dwntoshiPerByte = 1
    }
    ...
    return (
      (this.get(transaction.type) + transactionSizeInBytes) * dwntoshiPerByte
    )
  }
