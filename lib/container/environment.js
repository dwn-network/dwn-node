...
const { NetworkManager } = require('@dwn/extend')
...
module.exports = class Environment {
  ...
  __exportPaths() {
    ...
    for (const [key, value] of Object.entries(this.variables)) {
      if (allowedKeys.includes(key)) {
        process.env[`DWN_PATH_${key.toUpperCase()}`] = path.resolve(
          expandHomeDir(value),
        )
      }
    }
  ...
  __exportNetwork() {
    ...
    if (this.variables.token && this.variables.network) {
      ...
      try {
        const networkPath = path.resolve(
          expandHomeDir(`${process.env.DWN_PATH_CONFIG}/network.json`),
        )
    ...
    process.env.DWN_NETWORK = JSON.stringify(config)
    process.env.DWN_NETWORK_NAME = config.name
  }
  ...
  __exportVariables() {
    ...
    const envPath = expandHomeDir(`${process.env.DWN_PATH_DATA}/.env`)
