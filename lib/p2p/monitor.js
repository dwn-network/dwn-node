...
class Monitor {
  ...
  async updateNetworkStatus(networkStart) {
    if (process.env.DWN_ENV === 'test' || process.env.NODE_ENV === 'test') {
      return
    }
    ...
    if (!this.__isColdStartActive()) {
      logger.warn(
        'cold start activity is false, so skip check',
      )
      return
    }
  ...
  dumpPeers() {
    ...
    try {
      fs.writeFileSync(
        `${process.env.DWN_PATH_CONFIG}/peers_backup.json`,
